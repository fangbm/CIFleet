...
func TestCreateAddsIsolationLabelsAndRepositoryScopedCache(t *testing.T) {
	...
	instance, err := b.Create(context.Background(), backend.JobSpec{
		ID:         "job-1",
		...
		CPU:        2,
		MemoryM:    1024,
		CacheMounts: []backend.CacheMount{{
	...
	for _, wanted := range []string{
		"--label io.cifleet.managed=true",
		"--label io.cifleet.repository=fangbm/CIFleet",
		"--cpus 2",
		"--memory 1024m",
		"ubuntu:24.04 sleep 60",
	} {
...
func TestRejectsUnsafeCacheTarget(t *testing.T) {
	...
	_, err := b.Create(context.Background(), backend.JobSpec{
		ID: "job", Repository: "owner/repo", Image: "ubuntu", CacheMounts: []backend.CacheMount{{Name: "x", Target: "../escape"}},
	})
	if err == nil {
